...
![CLI 演示](images/cli_example.gif)

上图展示了 `search` + `--move-to` 的完整工作流：

1. 按关键词搜索专辑（`--type album`）— 出现交互式选择菜单
2. 用上下键选中目标专辑，回车确认
3. 歌词下载到 `./lyrics/`（自动创建「歌手 - 专辑名」子目录）
4. `--move-to` 扫描音乐目录，用模糊相似度将每个 LRC 文件与音频文件匹配（支持繁简互转），展示确认表格
5. 确认后，LRC 文件移动并重命名为对应音频文件的文件名
